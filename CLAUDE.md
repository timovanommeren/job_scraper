...
| BIT | `scrapers/bit.py` | requests + BS4, `article.c-list-item` cards + per-vacancy detail page |
...
- Change the health dashboard or its stats → `feedback/stats.py` (`compute_stats(conn) -> dict`, shared by `/dashboard` and `/api/v1/stats`); run-log aggregation is delegated to `db/dedup.py:get_run_stats()` (single source). Dashboard config (`analytics_window_days`, `scheduled_run_time`, `pricing`, `usd_to_eur`) is in `config/settings.yaml:dashboard`.
- Change API cost/token tracking → tokens are captured per call in `agents/extractor_scorer.py` via `create_with_completion()` (usage appended to a caller-owned `usage_sink`), summed in `main.py:score_new_jobs`, costed via `feedback/stats.py:compute_cost_eur()` (unknown model → `None`, no crash), and stored on `run_log` (`input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_creation_tokens`, `est_cost_eur`). Pricing map is `config/settings.yaml:dashboard.pricing`.
...
6. **Never re-enable a disabled scraper (uncareers) without reading its GitHub issue.** These are blocked by external infrastructure, not fixable with code changes alone. (OECD and BIT were both re-enabled 2026-06-22: OECD via its SmartRecruiters Posting API, BIT via its WordPress careers page — in both cases the Cloudflare block only ever applied to the old HTML entry point, and a probe found a reachable path behind it. The lesson: re-probe "Cloudflare-blocked" sources periodically; the block may be stale or path-specific.)
